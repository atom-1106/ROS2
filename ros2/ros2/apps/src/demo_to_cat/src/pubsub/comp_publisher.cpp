...
#include "demo_to_cat/comp_publisher.hpp"
...
: Node("comp_pub_node", options), count_(0)
{
  oper_ = true;
  // Reliability: Reliable, Durability: Volatile, History: Keep Last, Depth: 10
  rclcpp::QoS system_qos = rclcpp::SystemDefaultsQoS();
  publisher_ = create_publisher<demo_interfaces::msg::Vehicle>("comp_topic_name", system_qos);
...
RCLCPP_COMPONENTS_REGISTER_NODE(demo::PublisherNode)
