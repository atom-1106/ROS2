...
class RequestsServerLogic : public frpc::RpcServer, public std::enable_shared_from_this<RequestsServerLogic>
{
...
public:
	...
	void execute_request(std::shared_ptr<frpc::RpcRequest> const& request) override
	{
...
private:
	...
	struct IInputFeedProcessor
	{
		...
		virtual bool process_additional_request(RequestType const& request) = 0;
	...
	struct RequestContext : frpc::RpcRequest
	{
		...
		struct Write_feeds_t
		{
		} Write_feeds;

		frtps::GUID_t const& get_client_id() const override
		{
			return client_id;
		}

		frtps::RemoteLocatorList const& get_client_locators() const override
		{
			return pub_data.remote_locators;
		}

		fdds::ReturnCode_t take_from(frpc::Replier* replier)
		{
		...
		{
			size_t n_fields = 0;
			n_fields += request.Read.has_value() ? 1 : 0;
			n_fields += request.Write.has_value() ? 1 : 0;
		...
		)
		{
			...
			if(ctx->info.related_sample_identity == info.related_sample_identity)
			{
				...
				should_erase = true;
				for(auto const& input_feed : input_feed_processors_)
				{
		...
		bool prepare(frpc::Replier* replier)
		{
			if(request.Read.has_value())
			{
				return prepare_Read(replier);
			}

			if(request.Write.has_value())
			{
				return prepare_Write(replier);
			}
		...
		void send_exception(frpc::RemoteExceptionCode_t ex, frpc::Replier* replier)
		{
			ReplyType reply{};
			reply.remoteEx = ex;
	...
	private:
		...
		bool prepare_Read(frpc::Replier* replier)
		{
			static_cast<void>(replier);
			return true;
		}

		bool prepare_Write(frpc::Replier* replier)
		{
			static_cast<void>(replier);
			return true;
		}
	...
	struct ThreadPool : public frpc::RpcServerSchedulingStrategy
	{
		ThreadPool(RequestsServerLogic& server, size_t num_threads) : server_(server)
		{
			...
			auto process_requests = [this]()
			{
				while(!finished_)
				{
					std::shared_ptr<frpc::RpcRequest> req;
					{
						...
						cv_.wait(lock, [this]() { return finished_ || !requests_.empty(); });
						if(finished_)
						{
							break;
						}
		...
		void schedule_request(
		    std::shared_ptr<frpc::RpcRequest> const& req,
		    std::shared_ptr<frpc::RpcServer> const& server
		) override
		{
		...
		void server_stopped(std::shared_ptr<frpc::RpcServer> const& server) override
		{
			static_cast<void>(server);
	...
	void process_request(std::shared_ptr<RequestContext> const& ctx)
	{
	...
	void execute_request(std::shared_ptr<RequestContext> const& req)
	{
		try
		{
			for(;;)
			{
				if(req->request.Read.has_value())
				{
					try
					{
						ReplyType reply{};
						reply.Read                  = cat::middleware::dds::detail::Requests_Read_Result{};
						reply.Read->result          = cat::middleware::dds::detail::Requests_Read_Out{};
						reply.Read->result->return_ = implementation_->Read(*req, req->request.Read->req);
						replier_->send_reply(&reply, req->info);
					}
					catch(cat::middleware::dds::RequestException const& ex)
					{
						ReplyType reply{};
						reply.Read = cat::middleware::dds::detail::Requests_Read_Result{};
						reply.Read->cat_middleware_dds_RequestException_ex = ex;
						replier_->send_reply(&reply, req->info);
					}
					break;
				}

				if(req->request.Write.has_value())
				{
					try
					{
						ReplyType reply{};
						reply.Write                  = cat::middleware::dds::detail::Requests_Write_Result{};
						reply.Write->result          = cat::middleware::dds::detail::Requests_Write_Out{};
						reply.Write->result->return_ = implementation_->Write(*req, req->request.Write->req);
						replier_->send_reply(&reply, req->info);
					}
					catch(cat::middleware::dds::RequestException const& ex)
					{
						ReplyType reply{};
						reply.Write = cat::middleware::dds::detail::Requests_Write_Result{};
						reply.Write->cat_middleware_dds_RequestException_ex = ex;
						replier_->send_reply(&reply, req->info);
					}
					break;
				}
		...
		catch(frpc::RpcRemoteException const& ex)
		{
	...
	std::shared_ptr<RequestsServer_IServerImplementation> implementation_;
};

struct RequestsServerProxy : public frpc::RpcServer
{
	RequestsServerProxy(std::shared_ptr<frpc::RpcServer> impl) : impl_(std::move(impl))
	{
	}

	~RequestsServerProxy() override
	{
		if(impl_)
		{
			impl_->stop();
...
std::shared_ptr<eprosima::fastdds::dds::rpc::RpcServer> create_RequestsServer(
...
{
	auto ptr = std::make_shared<detail::RequestsServerLogic>(part, service_name, qos, thread_pool_size, implementation);
	return std::make_shared<detail::RequestsServerProxy>(ptr);
}
...
{
	auto ptr = std::make_shared<detail::RequestsServerLogic>(part, service_name, qos, scheduler, implementation);
	return std::make_shared<detail::RequestsServerProxy>(ptr);
}
